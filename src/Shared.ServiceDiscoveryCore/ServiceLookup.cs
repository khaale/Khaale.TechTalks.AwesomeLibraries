...
namespace ServiceDiscoveryCore
{
    ...
    public class ServiceLookup : IServiceLookup
    {
        ...
        public Uri GetServiceUri(string serviceName)
        {
            using (var client = new ConsulClient())
            {
                ...
                if (service == null)
                    throw new ArgumentException("Unable to find service in catalog");
    ...
    public class ServiceLookupFromAppConfig : IServiceLookup
    {
        public Uri GetServiceUri(string serviceName)
        {
            throw new NotSupportedException("Use Consul or DIE!!!!!");//return new Uri(ConfigurationManager.AppSettings[string.Format("services.{0}.uri", serviceName)]);
        }
    }
